import {
  ...
  iconPlay, iconPause, iconLoading, iconHeart, iconDownload, iconShare,
...
async function init() {
  ...
  let names, trans, slugs, cards;
  try {
    [names, trans, slugs, cards] = await Promise.all([
...
/* Replace the visible URL with the canonical slug form so every page
   ...
   bounce — ends up showing `/alloh/<slug>` in the address bar. We
